...
<body>
        ...
        <div class="page-inner">
            <div class="d-flex align-items-left align-items-md-center flex-column flex-md-row pt-2 pb-4">
                ...
                <div class="ms-md-auto py-2 py-md-0 float-end">
                    ...
                    <a class="btn btn-primary btn-round" data-bs-toggle="modal" data-bs-target="#exampleModal">Add Customer</a>
                </div>
            </div>
            <div class="row">
                    ...
                    <div class="modal-body">
                        <form>
                            ...
                            <div id="selected-services" class="mt-4">
                                <!-- Daftar layanan yang dipilih akan ditampilkan di sini -->
                            </div>

                            <div class="mb-3 row">
                                <label for="category_id" class="col-sm-4 col-form-label">Pilih Metode</label>
                                <div class="col-sm-8" id="metode-container">
                                    <!-- Radio button akan dimasukkan di sini dengan AJAX -->
                                </div>
                            </div>
        ...
        <div class="modal fade" id="quantityModal" tabindex="-1" aria-labelledby="quantityModalLabel" aria-hidden="true">
            <div class="modal-dialog modal-sm"> <!-- Kelas modal-sm untuk ukuran kecil -->
                <div class="modal-content">
                    ...
                    <div class="modal-footer">
                        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Close</button>
                        <button type="button" id="save-quantity" class="btn btn-primary">Simpan</button>
                    </div>
                    </form>
                </div>
            </div>
    ...
    $(document).ready(function () {
        // Ketika modal dibuka, panggil layanan dengan AJAX
        $('#layanan').on('show.bs.modal', function () {
            // Bersihkan kontainer sebelum memuat data baru
            $('#card-container').html('');

            // Lakukan permintaan AJAX ke server
            $.ajax({
                url: '/api/layanans', // Ganti dengan endpoint API Anda
                method: 'GET',
                dataType: 'json',
                success: function (response) {
                    // Iterasi hasil data dan tambahkan ke kontainer
                    response.forEach(function (layanan) {
                        const cardHtml = `
                            <div class="col-md-4 mb-3">
                                <div class="card">
                                    <img src="/storage/${layanan.thumbnail}" class="card-img-top" alt="${layanan.nama}">
                                    <div class="card-body">
                                        <h5 class="card-title">${layanan.nama_layanan}</h5>
                                        <p class="card-text">${layanan.jenis_layanan}</p>
                                        <button class="btn btn-primary pilih-layanan" data-id="${layanan.id}">Pilih</button>
                                    </div>
                                </div>
                            </div>
                        `;
                ...
                error: function () {
                    $('#card-container').html('<p class="text-danger">Gagal memuat data layanan.</p>');
                }
            });
        });
        ...
        $(document).on('click', '.pilih-layanan', function () {
            ...
            const layananNama = $(this).closest('.card').find('.card-title').text();
            const layananKategori = "Kategori Placeholder"; // Ganti dengan data kategori yang sesuai
            const layananJenis = "Jenis Layanan Placeholder"; // Ganti dengan data jenis layanan yang sesuai

            // Update informasi di modal quantityModal
            $('#layanan-name').text(layananNama);
            $('#layanan-category').text(layananKategori);
            $('#layanan-jenis_layanan').text(layananJenis);

            // Tampilkan modal kecil
            $('#quantityModal').modal('show');
        });

        // Ketika tombol 'Simpan' diklik di modal quantityModal
        $('#save-quantity').on('click', function () {
            const kuantitas = $('#jumlah').val();
            const layananNama = $('#layanan-name').text();

            if (kuantitas > 0) {
                alert(`Layanan: ${layananNama}, Kuantitas: ${kuantitas} berhasil disimpan.`);
                $('#quantityModal').modal('hide'); // Tutup modal
            } else {
